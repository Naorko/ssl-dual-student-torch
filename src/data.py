...
def relabel_dataset_dict(dataset, labels):
    labeled_idxs = {k: [] for k in dataset.class_to_idx.values()}
    unlabeled_idxs = {k: [] for k in dataset.class_to_idx.values()}
    ...
    for idx in range(len(dataset.imgs)):
        ...
        if filename in labels:
            dataset.imgs[idx] = img_path, label_idx
            del labels[filename]
