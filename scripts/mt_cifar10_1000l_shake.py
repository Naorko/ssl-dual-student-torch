...
def parameters():
    ...
    # 1000 labels:
    for data_seed in range(10, 11):  # TODO: Run more label files?
        yield {
            **defaults,
            'title': '1000-label cifar-10',
            'n_labels': 1000,
            'data_seed': data_seed,
            'epochs': 1,  # 180, # TODO: Run more epochs?
...
def run(title, base_batch_size, base_labeled_batch_size, base_lr, n_labels, data_seed, **kwargs):
    ...
    mean_teacher_baseline.args = parse_dict_args(**adapted_args, **kwargs)
    mean_teacher_baseline.main(context)
